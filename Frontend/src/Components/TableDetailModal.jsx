...
import { X, Printer, Download, MapPin, Users } from 'lucide-react';
...
const TableDetailModal = ({ table, onClose }) => {
  ...
  const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(clientUrl)}`;
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[10000] animate-in fade-in duration-200">
      <div className="bg-neutral-900 border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden relative">
        ...
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-white bg-black/20 p-2 rounded-full transition-colors">
          <X size={24} />
        </button>
        ...
        <div className="p-8 text-center">
          ...
          <h2 className="text-3xl font-black text-white mb-2">{table.table_number}</h2>
          <div className="flex justify-center gap-4 text-gray-400 text-sm mb-6">
            <span className="flex items-center gap-1"><Users size={16} className="text-orange-500"/> {table.capacity} Khách</span>
            <span className="flex items-center gap-1"><MapPin size={16} className="text-orange-500"/> {table.location}</span>
          </div>

          {table.description && (
            <p className="text-gray-500 italic mb-8 border-t border-white/5 pt-4">"{table.description}"</p>
          )}
