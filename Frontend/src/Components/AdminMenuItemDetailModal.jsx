...
export default function AdminMenuItemDetailModal({ open, item, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ gọi API detail
        const res = await adminMenuApi.getMenuItemDetail(itemId);
        const full = res?.item;
  ...
  // fallback: title vẫn dùng item list để nhìn đỡ trống
  const titleName = detail?.name ?? item?.name ?? "—";
  const view = detail ?? item; // nếu detail chưa về thì vẫn hiện tạm
