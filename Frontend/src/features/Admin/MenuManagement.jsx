...
import { 
  ...
  Image as ImageIcon, Star, X, Upload 
} from 'lucide-react';
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {items.map((item) => (
            <div key={item.id} className="bg-white rounded-xl shadow-sm border overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mb-3">
                   ...
                   <span className={`text-xs px-2 py-0.5 rounded border ${item.status === 'available' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                     {item.status === 'available' ? 'Có hàng' : 'Hết hàng'}
                   </span>
                </div>

                <p className="text-xs text-gray-500 line-clamp-2 h-8 mb-4">{item.description || "Chưa có mô tả"}</p>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[9999] backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
              ...
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 p-1 rounded-full">
                <X size={20} />
              </button>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input 
                        name="prep_time_minutes" type="number" min="0"
                        className="w-full text-gray-700 px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <input 
                      type="checkbox" id="chefReq"
                      name="is_chef_recommended"
                      checked={formData.is_chef_recommended}
                      onChange={handleChange}
                      className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
                    />
                    <label htmlFor="chefReq" className="text-sm font-medium text-gray-700 cursor-pointer">
