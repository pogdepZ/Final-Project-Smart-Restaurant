  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await axiosClient.post(
        `/admin/tables/${table.id}/regenerate`,
      );
      ...
      <div className="p-6 border-t border-white/10 bg-black/20 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="py-2.5 bg-neutral-800 hover:bg-neutral-700 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 border border-white/10"
          >
        ...
        <button
          onClick={handlePrint}
          className="w-full py-3 bg-orange-600 hover:bg-orange-500 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-orange-900/20"
        >
          <Printer size={18} /> In Ngay
        </button>
        <button
          onClick={handleRegenerate}
          className="w-full py-2.5 text-red-400 hover:bg-red-500/10 border border-red-500/30 rounded-xl font-bold text-xs flex items-center justify-center gap-2"
        >
