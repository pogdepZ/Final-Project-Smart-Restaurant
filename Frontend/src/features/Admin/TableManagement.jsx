...
import { 
  Plus, Search, Filter, Edit, Trash2, Printer, 
  Power, CheckCircle, AlertCircle, X, QrCode, MapPin, Users 
} from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 border-b border-white/10 pb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1 flex items-center gap-2">
            ...
            Hệ thống đang hoạt động • Tổng số: <span className="text-white font-bold">{tables.length}</span> bàn
          </p>
        </div>
            ...
            <div key={table.id} className={`group relative bg-neutral-900/50 border border-white/5 rounded-2xl p-6 transition-all hover:border-orange-500/30 hover:shadow-[0_0_20px_rgba(0,0,0,0.5)] ${table.status === 'inactive' ? 'opacity-60 grayscale-50' : ''}`}>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEdit(table)} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg">
                        <Edit size={16} />
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-6">
                    <div className="flex items-center gap-2 text-sm text-gray-400">
                        ...
                        <span>Sức chứa: <strong className="text-white">{table.capacity}</strong> khách</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-400">
                        <MapPin size={14} className="text-orange-500" />
                        <span>Khu vực: <span className="text-white">{table.location}</span></span>
                    </div>
                    {table.description && (
          ...
          <div className="bg-neutral-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="space-y-4">
                <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5 ml-1">Số bàn / Tên bàn</label>
                    <div className="relative">
                        <span className="absolute left-3 top-2.5 text-gray-500 font-bold">#</span>
                        <input 
                        name="table_number" type="text" required placeholder="VD: T-01"
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5 ml-1">Sức chứa</label>
                    <input 
                        name="capacity" type="number" required min="1" max="20"
                    ...
                    <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5 ml-1">Vị trí</label>
                    <select 
                    ...
                    >
                        <option value="" className="bg-neutral-900 text-gray-500">Chọn khu vực</option>
                        <option value="Indoor" className="bg-neutral-900">Indoor (Trong nhà)</option>
                        <option value="Outdoor" className="bg-neutral-900">Outdoor (Ngoài trời)</option>
                        <option value="Patio" className="bg-neutral-900">Patio (Sân thượng)</option>
                ...
                <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5 ml-1">Ghi chú</label>
                    <textarea 
